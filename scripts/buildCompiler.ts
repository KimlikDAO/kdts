...
const BazelCompilerJar = resolve("vendor", "gcc", "bazel-bin", "compiler_uberjar_deploy.jar");
...
const readCompilerVersion = (packageVersion: string): string => {
  try {
    const revision = capture(["git", "-C", "vendor/gcc", "rev-parse", "--short=12", "HEAD"]);
...
const ensureCompilerJar = (
  ...
  if (existsSync(BuiltCompilerJar))
    return BuiltCompilerJar;
  if (!existsSync("vendor/gcc/.bazelversion"))
    throw new Error("Missing vendor/gcc submodule. Run `git submodule update --init --recursive`.");
