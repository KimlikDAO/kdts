...
test("local @types win when a package is also installed", () => {
  expect(resolvePath("src/entry.ts", "uglify-js")).toEqual({
    path: "kdts/@types/uglify-js/index.d.ts",
...
test("repo package resolves through kdts.sources", () => {
  expect(resolvePath("src/entry.ts", "@kimlikdao/kdts")).toEqual({
    path: "kdts/kdts.d.ts",
    id: "package:@kimlikdao/kdts"
  });
});
